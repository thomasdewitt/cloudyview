...
export class Viewer {
  constructor(device, canvas, uiRoot) {
    ...
    // Prototype ice-detection mode (I in flight): false-color ice fraction
    // plus the dart-tip condensate readout. Off on every load.
  ...
  async loadField(source, progress) {
    ...
    // Prototype ice-detection mode carries across a change of field only if
    // the new field can support it.
    if (this.iceModeOn && !scene.iceView) this.iceModeOn = false;
    renderer.iceMode = this.iceModeOn;
  ...
  _onKeyDown(e) {
    ...
    switch (key) {
      ...
      case "i": this.toggleIceMode(); return;
  ...
  /**
   * I: prototype ice-detection mode — false-color ice fraction (liquid red
   * to ice cyan, alien green sky) plus the dart-tip condensate readout.
   */
  toggleIceMode() {
    if (!this.scene?.iceView) {
      this.ui.say(
        "Ice detection needs an ice mixing ratio variable (QI / IWC) in the " +
        "loaded field, and this one has none.", 5);
      return;
    }
    this.iceModeOn = !this.iceModeOn;
    this.renderer.iceMode = this.iceModeOn;
    this._syncIceReadout();
    this.ui.say(this.iceModeOn
      ? "Ice detection on — liquid burns red, ice glows cyan."
      : "Ice detection off.", 4);
    this._wake("iceMode");
  }
  ...
  async _frame(generation) {
    ...
    // Likewise the dart-tip condensate probe (prototype ice-detection mode).
    this._iceTick(now);
