...
struct Uniforms {
    ...
    // w = ice-detection mode (0.0 or 1.0), a PROTOTYPE false-color view:
...
// PROTOTYPE (ice-detection mode): per-voxel ice extinction fraction
...
fn fs_main(@builtin(position) frag_pos: vec4<f32>) -> @location(0) vec4<f32> {
    ...
    // PROTOTYPE ice-detection mode: 0.0 in every shipped preset, and every
    // branch on it below leaves the math bit-identical when off.
    ...
    if (t_near >= 0.0 && t_near < t_far) {
        for (var i: i32 = 0; i < max_view_steps; i = i + 1) {
            ...
            // PROTOTYPE ice-detection tint for this sample's cloud source
            // terms. Exactly 1.0 with the mode off, so the multiplies below
